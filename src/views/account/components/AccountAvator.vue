<template>
  <div class="account-avatar">
    ...
    <div class="avatar-content">
      ...
      <div class="img-clip-wrap">
        ...
        <CropperDefault
          ...
          :wrapHeight="state.wrapHeight"
          v-model:prevWidth="state.prevWidth"
          v-model:prevHeight="state.prevHeight"
          v-model:prevOffsetX="state.prevOffsetX"
          v-model:prevOffsetY="state.prevOffsetY"
          v-model:cutOrder="state.cutOrder"
          @cut="send"
        ></CropperDefault>
      ...
      <div class="img-preview-wrap">
        <div
          class="pre-container"
          :style="`width: ${state.wrapWidth}px; height: ${state.wrapHeight}px;`"
        >
          <img
            v-show="state.imgUrl"
            :src="state.imgUrl"
            alt=""
            :style="`width: ${state.prevWidth}px;
                                height: ${state.prevHeight}px;
                                transform: translateX(-${state.prevOffsetX}px) translateY(-${state.prevOffsetY}px);`"
          />
        </div>
    ...
    <p class="descript">
      请选择图片上传：大小180 * 180像素以上，支持JPG、PNG等格式，图片需小于5M
    </p>
    <div class="footer">
      <div class="btn" :class="{ enable: !isCurrAvatar }" @click="submit">
        更新
      </div>
    </div>
  </div>
...
onMounted(() => {
  if (useUser.userinfo) state.imgUrl = useUser.userinfo.avatarUrl;
});
watch(
  () => useUser.userinfo,
  (newVal) => {
    if (!newVal) return;
    state.imgURL = newVal.imgURL;
    init();
  },
);
const isCurrAvatar = computed(
  () =>
    !state.imgUrl ||
    state.imgUrl === useUser.userinfo.avatarUrl ||
    !useUser.userinfo.uid,
);
// 发送裁剪指令
const submit = () => {
  if (state.loading || isCurrAvatar.value) return;
  state.loading = true;
  state.cutOrder = true;
};
// 将收到的裁剪好的图片发送后端
    ...
    .then((blob) => {
      ...
      upDataAvatar(formData).then((res) => {
        if (res.code === 200) {
          ...
          useUser.setUpAvatarUrl(res.result);
        }
      });
    })
    .catch(() => {
...
const init = () => {
  state.prevWidth = state.wrapWidth;
  state.prevHeight = state.wrapHeight;
  state.prevOffsetX = 0;
  state.prevOffsetY = 0;
};
// 处理拖拽事件
const handlePicDrop = (event) => {
