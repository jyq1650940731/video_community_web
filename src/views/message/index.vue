 ...
 * @LastEditTime: 2024-06-18 16:10:59
 * @LastEditors: YourName
 * @Description: 
 * @FilePath: \video_community_web\src\views\message\index.vue
 * 版权声明
-->
<template>
        ...
        <div class="space-left">
          <div class="side-bar">
            ...
            <ul class="side-list">
              <li
                class="item"
                :class="{ active: routerIndex === index }"
              ...
              >
                <div class="text">
                  ...
                  <span
                    ...
                    v-if="
                      useUser.userinfo.uid && useMessage.msgUnread[index] > 0
                    "
                  >
                    {{
                      useMessage.msgUnread[index] <= 99
                        ? useMessage.msgUnread[index]
                        : '99+'
                    }}
                  </span>
                </div>
            ...
            <div class="setting">
              <div
                class="item"
                :class="{ active: routerIndex === 5 }"
                @click="router.push('/message/config')"
              >
                <svg
                  t="1701876122651"
                  class="icon"
        ...
        <div class="space-right">
          <div class="space-right-top">
            <div class="title">
              <span>
                {{
                  state.router[routerIndex]
                    ? state.router[routerIndex].name
                    : '消息设置'
                }}
              </span>
            </div>
...
<script setup lang="ts" name="MessageView">
import { useUserStore } from '@/stores/modules/user';
import { useMessageStore } from '@/stores/modules/message';
import backgroundImage from '@/assets/images/background/message-bg.png';
...
const routerIndex = computed(() => {
  let i = state.router.findIndex((item) => route.path.startsWith(item.path));
  if (i === -1) {
    return 5;
  } else {
    return i;
