<template>
  <div class="search-input">
    <div class="flex_center">
      <div
        class="search-input-wrap flex_between"
      ...
      >
        ...
        <div
          ...
          :style="state.input == '' ? 'display: none;' : ''"
          @click.stop="state.input = ''"
        >
          <el-icon size="20"><CircleCloseFilled /></el-icon>
        </div>
        <el-button
          class="v-ui-button v-ui-button--lg search-button"
          type="primary"
          @click="goSearch"
        >
          搜索
        ...
        <div
          class="search-panel"
          :style="state.isActive ? '' : 'display: none;'"
        >
          <div class="history" v-if="state.input == ''">
            <div class="header">
            ...
            <div
              class="history-fold"
              v-else
              @click.stop="state.isHistoryOpen = true"
            >
              <div class="fold-text">展开更多</div>
              <i class="iconfont icon-xiajiantou"></i>
            </div>
          </div>
          <div class="trending" v-if="state.input == ''">
            <div class="header">
          ...
          <div class="suggestions" v-if="state.input != ''">
            <div
              class="suggest-item"
              v-for="(item, index) in state.matchingWord"
              :key="index"
              v-html="highlightKeyword(state.input, item)"
              @click.stop="clickItemToSearch(item)"
            ></div>
          </div>
        </div>
...
onMounted(() => {
  ...
  if (route.query.keyword) state.input = route.query.keyword;
});
onBeforeUnmount(() => {
  // 页面结束渲染前销毁事件的监听器
...
const getMatchingWord = async () => {
  if (state.input && state.input.trim() === '') return;
  const keyword = encodeURIComponent(state.input); // 对特殊字符进行编译
...
const goSearch = () => {
  ...
  if (input === '') {
    ...
    if (route.path.startsWith('/search/user')) {
      router.push(`/search/user?keyword=${input}`);
    } else {
      router.push(`/search/video?keyword=${input}`);
    }
  }
};
...
const searchPopShow = () => {
  state.isActive = true;
  getMatchingWord();
  // console.log("显示热搜框: ", this.isActive);
};
// 隐藏搜索气泡框
const searchPopHide = () => {
