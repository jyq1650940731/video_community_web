<template>
  <div class="video-card__wrap">
    <router-link :to="`/video/${data.video.vid}`" target="_blank">
      <div class="video-card__image">
        <div class="video-card__image--wrap">
          <picture class="video-card__cover">
            <img :src="data.video.coverUrl" :alt="data.video.title" />
          </picture>
        </div>
          ...
          <div class="video-card__stats">
            <div class="video-card__stats--left">
              <span class="video-card__stats--item">
                <svg-icon name="player" width="18" />
                <span class="video-card__stats--text">
                  {{ handleNums(data.stats.play) }}
                </span>
              </span>
              <span class="video-card__stats--item">
                <svg-icon name="danmu" width="18" />
                <span class="video-card__stats--text">
                  {{ handleNums(data.stats.danmu) }}
                </span>
              </span>
            </div>
            <div class="video-card__stats__duration">
              {{ handleDuration(data.video.duration) }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="video-card__info--right">
        ...
        <div class="video-card__info--bottom">
          ...
          <router-link
            ...
            :to="`/space/${data.user.uid}`"
            target="_blank"
          >
            <svg-icon name="up" width="24" height="18"></svg-icon>
            <span class="video-card__info--author">
              {{ data.user.nickname }}
            </span>
            <span class="video-card__info--date">
              · {{ handleUploadDate(data.video.uploadDate) }}
            </span>
          </router-link>
        </div>
      </div>
