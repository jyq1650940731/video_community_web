<!--
 * @Author: YourName
 * @Date: 2024-06-05 11:24:06
 * @LastEditTime: 2024-06-05 11:24:06
 * @LastEditors: YourName
 * @Description: 
 * @FilePath: \video_community_web\src\components\danmu\DanmuBox copy.vue
 * 版权声明
-->
<template>
      ...
      <div :style="`height: ${boxHeight - 44}px; overflow: hidden;`">
        <div class="table-header">
          <div class="btn-time" @click="resort(sortType === 1 ? 2 : 1)">
            ...
            <span
              class="btn-icon"
              :style="sortType === 2 ? 'transform: rotate(180deg);' : ''"
            >
              <svg
                v-if="sortType === 1 || sortType === 2"
                t="1703521678193"
                class="icon"
          ...
          <div class="btn-text" @click="resort(sortType === 3 ? 4 : 3)">
            ...
            <span
              class="btn-icon"
              :style="sortType === 4 ? 'transform: rotate(180deg);' : ''"
            >
              <svg
                v-if="sortType === 3 || sortType === 4"
                t="1703521678193"
                class="icon"
                viewBox="0 0 1024 1024"
                version="1.1"
          ...
          <div class="btn-date" @click="resort(sortType === 5 ? 6 : 5)">
            ...
            <span
              class="btn-icon"
              :style="sortType === 6 ? 'transform: rotate(180deg);' : ''"
            >
              <svg
                v-if="sortType === 5 || sortType === 6"
                t="1703521678193"
                class="icon"
                viewBox="0 0 1024 1024"
                version="1.1"
            ...
            <li class="dm-item" v-for="(item, index) in dmList" :key="index">
              ...
              >
                <span class="dm-info-time">
                  {{ handleTimePoint(item.timePoint) }}
                </span>
...
export default {
  ...
  methods: {
    ...
    // 删除弹幕前的最后通牒
    beforeDelete(id) {
      ElMessageBox.confirm('是否确定删除该条弹幕?', {
    ...
    async deleteDanmu(id) {
      ...
      if (!res.data) return;
      this.$store.state.danmuList = this.$store.state.danmuList.filter(
        (item) => item.id !== id,
      );
      ElMessage.info('删除成功');
    },
